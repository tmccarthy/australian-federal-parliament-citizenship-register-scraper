...
object ListDocuments {
      ...
      referencesWithAbsolutePaths <- referencesWithRelativePaths.traverse { documentReference =>
        for {
          absoluteDocumentLocation <-
            IO.fromEither(ExceptionOr.catchIn(aphPage.resolve(documentReference.documentLocation)))
  ...
  private def onlyOrFail(elements: Elements): ExceptionOr[Element] =
    if (elements.size == 1) Right(elements.first())
    else Left(GenericException(s"Expected 1, but was ${elements.size()}"))
